...
# --------------------
# S5 "decision support" for the hydro space:
#   - build a hydro stratification (clusters/strata) over S3 segments
#     using hydro signatures already present in the S4 attrstack
#   - select representative candidate segments/points per stratum
#
# Key idea: "hydro_stratum" is an operational typification
# --------------------------------------------------------
# A stratum is a discrete partition of a continuous signature space.
# Here: hydro signatures (e.g. flow accumulation, Strahler, distance to water)
# are used to enforce coverage of different hydro-regimes in later decisions.
#
# IMPORTANT: domain restriction BEFORE selection
# ----------------------------------------------
# Candidates are searched ONLY within the gauge-station hull polygon:
#   - apply st_within() to segments early
#   - clustering + candidate selection operate only on within-hull segments
#
# Outputs
# -------
# - layer0_hydro_strata_segments: all segments with hydro_stratum fields attached
#   (outside hull -> NA, because hydro decision space is not evaluated there)
# - layer0_hydro_candidates_pts: candidate points (only inside hull by construction)
#######################################################################
...
## -------------------------------------------------------------------
## Parameters (same structure as physio)
## -------------------------------------------------------------------
n_per_stratum <- 5L
...
## -------------------------------------------------------------------
## Helpers (copied from physio; unchanged)
## -------------------------------------------------------------------
...
stopifnot("geom" %in% names(segs_sf_all))  # project convention
...
# Strict domain restriction: ONLY segments fully inside hull.
...
## -------------------------------------------------------------------
## 2) Hydro decision space (variables only differ from physio)
## -------------------------------------------------------------------
# These names match your attrstack naming convention (hydro__...__...).
hydro_cols <- c(
  "hydro__strahler__max",
  "hydro__flowacc__mean",
  "hydro__dist_to_water__mean"
)
...
stopifnot(nrow(hydro_df) > 0)

X  <- as.matrix(hydro_df[, hydro_cols, drop = FALSE])
Xz <- scale(X)
...
message("Hydro vars: ", length(hydro_cols), " total; ",
        length(keep_cols), " kept after |cor| <= ", cor_cutoff)

## -------------------------------------------------------------------
## 3) k-evaluation on a subsample (same logic as physio)
## -------------------------------------------------------------------
...
k_hydro <- choose_k_aggregate(k_suggestions, k_min = k_min, k_max = k_max)
message("Aggregated operational k_hydro: ", k_hydro)

## -------------------------------------------------------------------
## 4) Final k-means stratification (FULL, within hull, complete cases)
## -------------------------------------------------------------------
hydro_km <- kmeans(data_for_k, centers = k_hydro, nstart = kmeans_nstart_final)

centers_assigned <- hydro_km$centers[hydro_km$cluster, , drop = FALSE]
hydro_dist <- sqrt(rowSums((data_for_k - centers_assigned)^2))
...
## -------------------------------------------------------------------
## 5) Attach strata back to segments (all segments product)
## -------------------------------------------------------------------
hydro_seg_sf_all <- segs_sf_all %>%
  dplyr::left_join(hydro_assign, by = "segment_id")

hydro_seg_sf_in <- segs_sf %>%
  dplyr::left_join(hydro_assign, by = "segment_id")
...
hydro_candidates_seg <- hydro_seg_sf_in %>%
  dplyr::semi_join(hydro_candidates_ids, by = "segment_id") %>%
  dplyr::select(segment_id, hydro_stratum, hydro_dist_to_center, geom)

## -------------------------------------------------------------------
## 7) Candidate points (planar for point_on_surface)
## -------------------------------------------------------------------
hydro_candidates_pts <- hydro_candidates_seg %>%
  sf::st_make_valid() %>%
  sf::st_transform(crs_proj) %>%
  sf::st_point_on_surface() %>%
  sf::st_transform(sf::st_crs(hydro_candidates_seg)) %>%
  dplyr::mutate(space = "hydro") %>%
  dplyr::select(
    segment_id,
    space,
    stratum = hydro_stratum,
    dist_to_center = hydro_dist_to_center,
    geom
  )
...
sf::st_write(
  hydro_candidates_pts,
  dsn = hydro_pts_out,
  layer = "hydro_candidates_pts",
  driver = "GPKG",
  delete_dsn = TRUE,
  quiet = TRUE
)

message("Wrote segments:  ", hydro_seg_out, " (rows=", nrow(hydro_seg_sf_all), ")")
message("Wrote candidates:", hydro_pts_out, " (rows=", nrow(hydro_candidates_pts), ")")
